...
########################################
# MODEL 4.4: Logistic  Regression (V, T, M, Y, R)
# setup cross validation and control parameters
model4.4 <- train(AveragePrice ~.,
                  data = new_train_data.all, 
                  method = "glm",
                  trControl = trainControl("cv", number = 10),
                  tuneLength = 10)

# MODEL 4.4: Validate model: (V, T, M, Y, R)
model4.4.validation <- model4.4 %>% predict(new_test_data.xy) %>% as.vector()
model4.4.RMSE <- RMSE(new_test_data$AveragePrice, model4.4.validation)
model4.4.R2 <- R2(model4.4.validation, new_test_data$AveragePrice)

cat("Model 4.4 - Logistic Model: RMSE =", model4.4.RMSE)
cat("Model 4.4 - Logistic Model: R-squared =", model4.4.R2)

# MODEL 4.4: Plot results
model4.4.plot1 <- build_validation_plot(new_test_data.all, 
                                        model4.4.validation, 
                                        "MODEL 4.4 - Logistic  Regression",
                                        model4.4.RMSE,
                                        model4.4.R2)
...
results <- data.frame(
  c("MODEL 1 - MEAN Model", 
    "MODEL 2 - Linear Regression",
    "MODEL 3.3 - Multi Linear Regression", 
    "MODEL 4.2 - Lasso regression",
    "MODEL 4.4 - Logistic regression"),
  c(model1.RMSE, 
    model2.RMSE, 
